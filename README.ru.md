...
Приложение работает в связке с веб-сервером Nginx. Пользователь создает папки с файлами в рабочем каталоге. Эти папки становятся «раздачами». Процесс индексации сканирует эти папки и генерирует для них веб-страницы, сохраняя их вместе со счетчиками в Redis. Ссылка на раздачу представляет собой хеш от пути к папке.
...
### Метод 1: Docker (Рекомендуемый)

Это наиболее простой и быстрый способ запустить приложение.

1.  Создайте файл конфигурации `config.yml` (см. раздел [Конфигурация](#конфигурация)).
2.  Создайте файл `deploy/docker-compose.yml`.
3.  Запустите контейнеры с помощью команды, указав путь к вашим файлам в переменной `SHARE_PATH`:
    ```bash
    SHARE_PATH=/путь/к/вашим/папкам docker compose -f deploy/docker-compose.yml up --build
    ```
